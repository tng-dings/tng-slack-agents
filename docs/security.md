...
- The `OpenCodeServer` launcher (`scripts/Start-OpenCode.ps1`) applies a runtime-inline OpenCode policy that disables auto-update and plugins and denies external-directory access, web tools, subagents, skills, and interactive questions; unknown tools require approval and are automatically rejected. Shell/edit access remains because this is a coding worker. This policy is a property of the Windows service launcher, not of the runner: an OpenCode server started by hand (as in the README's interactive two-terminal setup) runs without it and is therefore suitable only for a disposable repository.
