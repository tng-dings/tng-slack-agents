...
- The Claude SDK child receives only a fixed allowlist of Anthropic, Bedrock, Vertex, and Foundry provider variables plus the ordinary restricted child environment; Slack and Discord credentials are omitted. The runner enforces Claude's subprocess credential scrubbing for Bash, hooks, and MCP servers and includes provider credential values in output and audit redaction. These are defense-in-depth controls, not a boundary between identities.
