...
7. Confirm the app is DM-only for the MVP, will initially allowlist a single named tester's Slack member ID, and must not be invited into channels or multi-person DMs.
8. Confirm that messages sent to the app—including source snippets—leave Slack for company-managed local compute and may be sent onward to the separately approved model provider through OpenCode.
9. Approve the stated governance: one concurrent job per user and globally, three queued jobs per user, 30-minute timeout, daily cost cap of 5 currency units as reported by OpenCode, and audit logging of user ID, Slack thread, prompt, tool events, output, usage, cost, and failures.
10. Agree on who may access the sensitive local audit data and confirm the proposed 30-day retention. Automated deletion is not yet part of the MVP, so the operator must own that procedure.
